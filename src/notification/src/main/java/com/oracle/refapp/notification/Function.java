...
public class Function extends OciFunction {
    ...
    private String getAccessToken(String clientId, String clientSecret) {
        ...
        Map<String, String> data = Map.of("grant_type", "client_credentials",
                "scope", apigwUrl + "/service");

        HttpRequest<?> request = HttpRequest.POST("/oauth2/v1/token", data).contentType(MediaType.APPLICATION_FORM_URLENCODED_TYPE)
                .header("Authorization", "Basic " + base64encodedSecret);
        Map<String, String> response = idcsClient.toBlocking().retrieve(request, Argument.mapOf(String.class, String.class));
    ...
    private Map<String, Object> getPatientDetails(String patientId, String accessToken) {
        ...
        HttpRequest<?> request = HttpRequest.GET("/v1/patients/" + patientId)
