...
class FollowupServiceTest {
  ...
  void testProcessMessageFromEncounterStream() throws InterruptedException, IOException {
    ...
    Map<String, Object> followupMessageMap = objectMapper.readValue(
      toStreamList.get(0).getPayload().toString(),
      new TypeReference<>() {}
    );
    Assertions.assertTrue(followupMessageMap.containsKey("messageType"));
    Assertions.assertTrue(followupMessageMap.containsKey("messageType"));
    Assertions.assertEquals("followup", followupMessageMap.get("messageType").toString());
    Assertions.assertEquals("patient@uho.com", followupMessageMap.get("patientEmail").toString());
    Assertions.assertEquals("UHO Patient", followupMessageMap.get("patientName").toString());
    Assertions.assertEquals("UHO Provider", followupMessageMap.get("providerName").toString());
  }
}
